...
print('Change made 28_05 at 11.03')
...
infile_df_gare='f_07_df_gare.pickle'
infile_df_risultati_gare='f_08_df_risultati_gare.pickle'
infile_db_gare_trattate='f_05_db_gare_trattate.pickle'
...
######LAVORO SUI DATI CONTENUTI IN DB GARE PRIMA DI METTERLI NEL DATA FRAME
#####DA PULIRE LE DATE
...
#!!!!!!! QUESTA VA RIVISTA: LA DATA FUTURA E' RELATIVA AL PROBLEMA DELLE EDIZIONI
date_ok = pd.to_datetime(date_pulite, format='%d-%m-%Y')
filtered_dates = date_ok[date_ok > '2024-04-12']
...
#####PROCEDURA PER TOGLIERE I CAMPI IN PIU' SE SONO PIU' DI NOVE
...
####HO FINITO DI TRATTARE DB_GARE ADESSO AGGIORNO I DF
##carico i df esistenti
df_gare=pickle.load(open(percorso+infile_df_gare,'rb'))
df_risultati_gare=pickle.load(open(percorso+infile_df_risultati_gare,'rb'))

df_aggiornamento_gare=pd.DataFrame({'nome_gara':nome_gare,'luogo':luoghi_raw,'comune':comune_raw,
                      'provincia':sigla_pr_raw,'data':date_ok,'tipo':tipo,
                      'url':url,'n_distanze':n_distanze})

df_gare=pd.concat([df_gare,df_aggiornamento_gare])
...
luoghi_unici=np.unique(df_gare.luogo)
luoghi_unici=[a for a in luoghi_unici if a!='']
sigla_pr_unici=np.unique(df_gare.provincia)
comuni_unici=np.unique(df_gare.comune)
date_uniche=np.unique(df_gare.data)
#list(filter(lambda x: len(x)>2 and x[-3]!=' ',luoghi_unici))
#sigla_pr=list(map(lambda x: x[-2:] if len(x)>2 and x[-3]==' ' else '' ,luoghi_unici))
tipo_ana=np.unique(df_gare.tipo)

#recupero il db gare trattate e lo aggiorno con quelle attuali
db_gare_trattate=pickle.load(open(percorso+infile_db_gare_trattate,'rb'))
db_gare_trattate.extend(db_gare)
pickle.dump(db_gare,open(percorso+'f_05_db_gare_trattate.pickle','wb'),protocol=4)

def genera_nome(gara,distanza):
    nuovo_nome=gara+distanza
    nuovo_nome=''.join(c for c in nuovo_nome if c.isalnum())
    nuovo_nome=nuovo_nome+'.xls'        
...
cont=0
##bisognerebbe tenere conto solo dei nuovi file non caricati
...
for gg in range(len(db_gare[:])):
    ...
    for dd in range(len(db_gare[gg][-2]))[:]:
        ...
        try:
            ...
            #se il campo nome è vuoto per tutti gli atleti non importo il file
            if all(transito.NOME.isnull()):
                print('nome tutto nullo')
                continue
            #in alcuni casi invece di TEMPO_UFFICIALE c'è TEMPO_REALE. lo sostituisco in caso
            transito.columns = transito.columns.str.replace('TEMPO_REALE', 'TEMPO_UFFICIALE')
            ...
            transito['tempo_vs_1']=numeratore/numeratore[0]
...
'https://www.endu.net/it/events/run-rome-the-marathon/results'
'https://www.endu.net/it/events/corri-per-la-befana-2/results'#solo pdf
'https://www.endu.net/it/events/xmilia/results'
...
#anche le anagrafiche esistenti

#DA VEDERE ANCHE
'https://www.icron.it/newgo/#/classifiche'
'https://www.icron.it/newgo/#/classifica/20233203'
'https://www.endu.net/it/events/napoli-city-half-marathon/results'
